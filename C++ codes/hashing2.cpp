...
class Hashing {
    ...
    void insert(int key) {
        for(int i = 0; i < table_size; i++) {
            if(hashtable[(hashindex(key, table_size)+i)%table_size] == 0) {
                hashtable[(hashindex(key, table_size)+i)%table_size] = key;
                return;
            }
        }
    ...
    void del(int key) {
        for(int i = 0; i < table_size; i++) {
            if(hashtable[(hashindex(key, table_size)+i)%table_size] == key) {
                hashtable[(hashindex(key, table_size)+i)%table_size] = 0;
                return;
            }
        }
        cout<<"void deletion";
